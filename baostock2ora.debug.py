...
class Downloader(object):
    ...
    def run(self):
	    ...
	    # adjustflag：复权类型，默认不复权：3；1：后复权；2：前复权。
            ...
            df_code['VOLUME'] = df_code['VOLUME'].astype("float")
            df_code['AMOUNT'] = df_code['AMOUNT'].astype("float")
            df_code['VOLUME'] = df_code['VOLUME']*0.00001
            df_code['AMOUNT'] = df_code['AMOUNT']*0.00001
            df_code.to_csv("D:\\demo_assignDayData.csv", encoding="gbk", index=False)
...
if __name__ == '__main__':
    ...
    downloader = Downloader(date_start='1990-12-19', date_end='2020-08-25')
    downloader.run()
