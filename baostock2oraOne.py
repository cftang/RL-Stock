...
if __name__ == '__main__':
    code = 'SZ300253'
    # with engine.connect() as connection:
    #     stmt = text("DELETE FROM hq_baostock WHERE code = :val")
    #     connection.execute(stmt,{"val": code})

    # 获取全部股票的日K线数据
    #now=datetime.now()
    #t = now.strftime("%Y-%m-%d")
    #downloader = Downloader(date_start=t, date_end=t)
    code = code[0:2].lower()+"."+code[2:8]
    downloader = Downloader(date_start='1990-12-19', date_end='2030-07-12')
    downloader.run(code)
